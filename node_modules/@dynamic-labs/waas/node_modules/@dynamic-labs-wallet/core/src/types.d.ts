...
export type IframeRequestMessages = {
    ...
    getClientKeyShare: (request: GetClientKeyShareRequest) => Promise<GetClientKeyShareResponse>;
    setClientKeyShare: (request: SetClientKeyShareRequest) => Promise<void>;
};
 ...
 *
 * The chainName parameter is required to distinguish between different chain types
 * that might use the same address format (e.g., EVM and SVM both use hex addresses).
 */
export interface SecureStorageAdapter {
    getClientKeyShare(chainName: string, accountAddress: string): Promise<unknown[]>;
    setClientKeyShare(chainName: string, accountAddress: string, shares: unknown[]): Promise<void>;
}
