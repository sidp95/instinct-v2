...
const OnrampContextProvider = ({ children, }) => {
    ...
    const [shouldFetchOnramps, setShouldFetchOnramps] = React.useState(true);
    ...
    const setEnabledOnrampProvidersTargetValue = React.useCallback(({ address, token, network, tokenAmount, chainName, currency, }) => {
    ...
    const enabledOnrampProvidersInDashboardOptions = React.useMemo(() => {
        const options = enabledOnrampProvidersInDashboard.map((provider) => {
            const option = onrampConfigurationToOnrampOption.onrampConfigurationToOnrampOption(provider);
            // Route Crypto.com selection to a custom UI instead of immediate popup/link
            if (option.id === sdkApiCore.ProviderEnum.CryptoDotCom) {
    ...
    const openOnramp = React.useCallback((_f) => _tslib.__awaiter(void 0, [_f], void 0, function* ({ onrampProvider, address, token, tokenAmount, network: networkOverride, chainName, currency, overrideOnRamp, payingWithDynamic, merchantName, }) {
        ...
        if (payingWithDynamic && !allPayingParameters) {
            rejectPromiseRef.rejectPromiseRef(promiseRef, new Error('PayingWithDynamic: Missing parameters'));
        ...
        if (payingWithDynamic && !onrampOption) {
            rejectPromiseRef.rejectPromiseRef(promiseRef, new Error('PayingWithDynamic: Onramp provider not found'));
            return (_h = promiseRef.current) === null || _h === void 0 ? void 0 : _h.promise;
        }
        if (!onrampOption) {
            rejectPromiseRef.rejectPromiseRef(promiseRef, new Error('Onramp provider not found'));
