...
const useEmbeddedWalletSessionKeys = ({ environmentId, projectSettings, }) => {
    ...
    const getDecodedSessionKeys = React.useCallback(() => {
    ...
    const registerHelper = React.useCallback((_a) => _tslib.__awaiter(void 0, [_a], void 0, function* ({ walletId, environmentId, privateKey, privateKeyJwk, publicKey, prevSessionKeySignature, user, }) {
        let resp;
     ...
     */
    const registerEmbeddedWalletSessionKey = React.useCallback((_b) => _tslib.__awaiter(void 0, [_b], void 0, function* ({ ignoreRestore = false, walletId, }) {
        if (!user) {
    ...
    const getSessionPublicKey = React.useCallback(() => {
    ...
    const shouldRegisterSessionKeysOnSignin = React.useCallback(() => true, []);
