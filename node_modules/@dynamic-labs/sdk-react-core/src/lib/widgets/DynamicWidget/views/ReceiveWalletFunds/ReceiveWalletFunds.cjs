...
var createFundingButtons = require('./utils/createFundingButtons/createFundingButtons.cjs');
var parseQuickSuggestions = require('./utils/parseQuickSuggestions/parseQuickSuggestions.cjs');
var useFundingHandlers = require('./utils/useFundingHandlers/useFundingHandlers.cjs');
var useFundingDisplay = require('./utils/useFundingDisplay/useFundingDisplay.cjs');
...
const ReceiveWalletFunds = ({ amount: initialAmount, token: initialTokenRaw, wallet, exchange, fundingWithExchange = false, accounts, quickSuggestions = defaultQuickSuggestions, network, destinationAddress, onBack, hideBackButton = false, }) => {
    ...
    const { secondaryDisplay, grayOutSecondaryDisplay, inputSymbolProp } = useFundingDisplay.useFundingDisplay({
        primaryData,
        secondaryData,
    });
    const quickSuggestionsWithTokenTemp = Array.isArray(quickSuggestions)
        ? { token: 'USD', values: quickSuggestions }
        : quickSuggestions;
    const { quickSuggestionsWithToken, quickSuggestionsParsed } = parseQuickSuggestions.parseQuickSuggestions(quickSuggestions, (value) => respectsMinimum.respectsMinimum({ token: quickSuggestionsWithTokenTemp.token, value: value.toString() }, minimum, exchangeRates));
    ...
    const { handleSelectToken, handleQuickSuggestion, closeTokenSelect, switchPrimaryAndSecondary, } = useFundingHandlers.useFundingHandlers({
        fiatAmount,
        inputEmitter,
        quickSuggestionsToken: quickSuggestionsWithToken.token,
    ...
    const { backButton, closeButton } = createFundingButtons.createFundingButtons({
        defaultBackAction: () => setDynamicWidgetView('choose-wallet-funding-method'),
        hideBackButton,
        onBack,
        setShowAuthFlow,
                    ...
                    'fund-from-wallet__token-select--open': showTokenSelect,
                        ...
                        : t('dyn_wallet_funding.from_wallet.amount_input.title') }) }), jsxRuntime.jsxs("div", { className: 'fund-from-wallet__content', children: [jsxRuntime.jsxs("div", { className: 'fund-from-wallet__content__amount-container', children: [minimum && (jsxRuntime.jsx(Typography.Typography, { color: 'error-1', className: classNames.classNames('fund-from-wallet__content__amount-container__minimum-error', {
                                    ...
                                    'fund-from-wallet__content__wallet-card__balance--disable-select': disableTokenSelect,
                                                    ...
                                                    value: (_g = primaryBalance.value) !== null && _g !== void 0 ? _g : consts.UNAVAILABLE_VALUE,
                                                    ...
                                                    value: (_h = secondaryBalance.value) !== null && _h !== void 0 ? _h : consts.UNAVAILABLE_VALUE,
