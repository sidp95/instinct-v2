...
const mapWalletsForRevoke = (wallets) => wallets
    .filter((w) => w.status === 'delegated')
    .map((wallet) => ({
...
// Exported handler factory functions for testing
const createHandleRevoke = (revokeDelegation) => (walletWithStatus) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
    try {
        const formattedWallets = mapWalletsForRevoke(walletWithStatus);
        yield revokeDelegation(formattedWallets);
        return true;
    }
    catch (error) {
        logger.logger.warn('Error revoking delegation:', error);
        return false;
    }
});
const createHandleDelegate = (delegateKeyShares) => (walletWithStatus) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
    try {
        const formattedWallets = mapWalletsForDelegate(walletWithStatus);
        yield delegateKeyShares(formattedWallets);
        return true;
    }
    catch (error) {
        logger.logger.warn('Error delegating key shares:', error);
        return false;
    }
});
const createHandleWalletStatusChange = (handleRevoke, handleDelegate) => (wallet) => _tslib.__awaiter(void 0, void 0, void 0, function* () {
    if (wallet.status === 'delegated') {
        yield handleRevoke([wallet]);
    }
    else {
        yield handleDelegate([wallet]);
    }
});
const createHandleRevokeAll = (handleRevoke, walletsWithStatus) => () => _tslib.__awaiter(void 0, void 0, void 0, function* () {
    yield handleRevoke(walletsWithStatus);
});
const WalletsDelegatedSettingsView = ({ name }) => {
    ...
    const { revokeDelegation, delegateKeyShares } = useWalletDelegation.useWalletDelegation();
    ...
    const handleRevoke = createHandleRevoke(revokeDelegation);
...
exports.createHandleDelegate = createHandleDelegate;
exports.createHandleRevoke = createHandleRevoke;
exports.createHandleRevokeAll = createHandleRevokeAll;
exports.createHandleWalletStatusChange = createHandleWalletStatusChange;
exports["default"] = WalletsDelegatedSettingsView;
exports.mapWalletsForDelegate = mapWalletsForDelegate;
exports.mapWalletsForRevoke = mapWalletsForRevoke;
